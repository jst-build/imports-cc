# Library `cares` for the `jst` build system

Target definitions for building `cares` (c-ares) from source. By default,
...
To import `cares` to your repository, add the following code to the
...
- `ALL`/`INSTALL`: Installs the c-ares library into an installation-style
  tree.
- `ares`: Target for consuming c-ares, an asynchronous DNS resolution
