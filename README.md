# System library `libssh2` for the `jst` build system

Target definitions for building against `libssh2` from the local system.
...
> Note that you must set `ENV["PATH"]` if `pkg-config` is not available in
