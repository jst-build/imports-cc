# System library `absl` for the `jst` build system

Target definitions for building against `absl` (Abseil) from the local
system. These targets perform `pkg-config` lookups and do not compile
anything.
