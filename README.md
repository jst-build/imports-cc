# Library `nlohmann_json` for the `jst` build system

Target definitions for building `nlohmann_json` (JSON for Modern C++)
from source. By default, this library is built with the system
toolchain. To use a different toolchain see [Repository
...
To import `nlohmann_json` to your repository, add the following code to
the *imports section* of your `repos.in.json` and run `jst-lock` to
generate the final repository lock-file

```jsonc
"imports": [
  {
    "source": "git",
    "branch": "nlohmann_json/v3.11.3",
    "url": "https://github.com/jst-build/imports-cc",
    "repos": [{"alias": "nlohmann_json"}]
  },
  // ...
],
```
...
- `ALL`/`INSTALL`: Installs the JSON for Modern C++ library into an
  installation-style tree.
- `json`: Target for consuming JSON for Modern C++ as a build dependency.
