# Library `cli11` for the `jst` build system

Target definitions for building `cli11` (CLI11) from source. By default,
...
To import `cli11` to your repository, add the following code to the
...
- `ALL`/`INSTALL`: Installs the CLI11 library into an installation-style
  tree.
- `cli11`: Target for consuming CLI11 as a build dependency. Header-only;
  nothing is compiled when consumed.
